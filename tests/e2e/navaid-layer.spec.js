...
test("navaid layer: toggle, click popup, and close on map click", async ({ page }) => {
  await page.goto("/map-harness.html?manifestUrl=/tests/fixtures/manifest.navaids.json");
  await page.locator("body[data-map-ready='true']").waitFor();

  const navaidToggle = page.locator("input[data-layer-id='faa-navaids']");
  await expect(navaidToggle).toBeVisible();
  await navaidToggle.check();

  const clickPixelHandle = await page.waitForFunction(() => {
    const app = document.querySelector("tile-viewer-app");
    const map = app?.map;
    const layer = app?.layerMap?.get("faa-navaids");
  ...
  const popup = page.locator("[data-role='navaid-popup']");
  await expect(popup).toBeVisible();
  await expect(popup).toContainText("Ident");
  await expect(popup).toContainText("Name");
  await expect(popup).toContainText("Frequency");
...
test("navaid layer: navaid popup has priority over obstacle popup", async ({ page }) => {
  await page.goto(
    "/map-harness.html?manifestUrl=/tests/fixtures/manifest.obstacles-and-navaids.json"
  );
  await page.locator("body[data-map-ready='true']").waitFor();
  ...
  const clickPixelHandle = await page.waitForFunction(() => {
    const app = document.querySelector("tile-viewer-app");
    const map = app?.map;
    const navaidLayer = app?.layerMap?.get("faa-navaids");
    const mapElement = app?.querySelector("[data-role='map']");
    if (!map || !navaidLayer || !navaidLayer.getVisible()) {
      return null;
    }
    const size = map.getSize();
    if (!size || !mapElement) {
  ...
  const navaidPopup = page.locator("[data-role='navaid-popup']");
  const obstaclePopup = page.locator("[data-role='obstacle-popup']");
  await expect(navaidPopup).toBeVisible();
  await expect(obstaclePopup).toBeHidden();
});
